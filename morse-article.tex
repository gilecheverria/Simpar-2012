...
The USARSim (Unified System for Automation and Robot Simulation)\cite{usarsim-4209284} shares many design concepts with MORSE. It
...
middlewares. USARSim use the Unreal Development Kit (UDK) that is a closed-source library and is not
...
customised robots and environments, but the interface to construct new
...
Elevation Map (DEM) files and create the appropriate 3D meshes. Additionally using aerial
...
robotics algorithms. Users can customise the existing components according to
their needs, or develop new ones when necessary, by describing their behaviour
in Python scripts.
