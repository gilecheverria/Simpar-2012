...
\begin{abstract}
  ...
  We present in this paper the current state of the simulator, highlighting
  its unique features with examples of the use cases where it has been used to
  validate several robotics experiments.
\end{abstract}
...
Land robots can have realistic wheel behaviour and physics, consisting of rigid
bodies for the wheels attached by physics constraints to the chassis of the
robot (Segway RMP 400 and Pioneer 3-DX). Alternatively, some robots are handled
as rigid boxes sliding over the ground (iRobot ATRV). For aerial and
submarine robots the physics are limited to collision detection, but they are
not affected by gravity, and there is no simulation of air or wind currents.
...
This flexibility permits scientists to focus on specific problems, by using
abstract data and avoiding data processing steps that are necessary but not the
main objective of the experiments.  \serge{I don't understand the point here.}
