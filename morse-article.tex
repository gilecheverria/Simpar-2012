...
\institute{
    ...
    \and
    Intelligent Autonomous Systems lab, Technische Universit\"{a}t M\"{u}nchen,
...
components and robots is unintuitive and complex, requiring searching though
data trees to adjust physical and geometrical parameters.
%\serge{Why the interface is complex ?}
...
been described in \cite{5980252}. Here we present a quick overview of
those principles.
...
video cameras.
To make a simulations closer to real life experiments, it is necessary
to create detailed models of the buildings and terrain where the
experimentations are carried out.
Transforming the 2D plan of a building into a 3D model is straightforward in
...
aerial photography for textures creates simulated environments that are very
similar to the real ones. Figure~\ref{fig:models} shows the modeling
capabilities of Blender being used to create the MORSE robots and an
environment created from DEM data.
...
\begin{itemize}
  \item Robots in MORSE have no behaviors. They are only define by a physical
    ...
    function.
    \serge{Are robots really components ?} \gilberto{Internally they are handled the same as other components, but they don't have associated actions at the moment}
  ...
  \item Actuators carry out actions within the simulated
...
be in charge of controlling a limited number of robots. The movements
...
synchronizing the time and events across all nodes. It can also be used
...
\serge{How you could accelerate the time without compromising the
  quality ?}
