...
experimentations are carried out. It is relatively straightforward using
Blender to transform a 2D plan of a building into a 3D model. However, for
experiments in \emph{field robotics} outdoor terrains are more difficult to
create by hand. We have developed additional plug-ins for Blender that can
read Digital Elevation Map (DEM) files and create the 3D mesh. Additionally
using aerial photography for textures creates simulated environments that are
very similar to the real ones.  Figure \ref{fig:models} shows the modelling
capabilities of Blender being used
...
We have presented the new features that have been developed on the MORSE
simulator.

These features have been developed following the requirements of a variety of
projects in robotics research. The design and architecture of MORSE has proved
to be flexible and powerful enough to allow researchers to use it under various
circumstances to test their robotics algorithms.  The new developments done on
top of MORSE are made available to the whole community, thanks to the open
source nature of the simulator.
