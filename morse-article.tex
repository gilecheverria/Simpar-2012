...
\begin{itemize}
  \item Robots in MORSE do nothing by themselves, only define the physical
    ...
    then used by the Bullet physics engine to compute the interactions with the
    environment.
  ...
  \item Actuators are charged with carrying out actions within the simulated
    ...
    \cite{iTaSC} solver available in the BGE).
\end{itemize}
...
can directly configure MORSE using Python scripts.
...
MORSE has been chosen as the simulator platform for the \emph{ANR-PROTEUS}
project, thanks to its versatility and the facility to integrate it with
existing architectures.
The PROTEUS project consists on the development of an integrated platform that
can be used by many French laboratories to test new robotics developments,
mainly for land and air robots.
...
software. This generates a lot of data, and can slow down the simulation when
...
robots. In order to be usable with any architecture, MORSE keeps a
...
These features have been used to connect MORSE with an existing Aerial
Unmanned Vehicle (AUV) control architecture~\cite{barbier2011} without
modifying the code in either the base architecture or MORSE.
...
display of the graphics. A scenario with several robots, video cameras and
